...
const BirthdayCardApp = () => {
  ...
  const CardEditor = () => (
    <View style={styles.cardContainer}>
      ...
      <TextInput
        style={[styles.textInput, { fontWeight: currentCard.isBold ? 'bold' : 'normal', fontSize: currentCard.fontSize }]}
        placeholder="Write your birthday message..."
        value={currentCard.text}
        onChangeText={(text) => setCurrentCard(prev => ({ ...prev, text }))}
        multiline
  ...
  const CardList = () => (
    <FlatList
      data={cards}
      keyExtractor={item => item.id.toString()}
      numColumns={Dimensions.get('window').width > 600 ? 2 : 1} // Show grid on larger screens
      renderItem={({ item }) => (
        <View style={styles.cardPreview}>
          {item.image && <Image source={{ uri: item.image }} style={styles.previewImage} />}
          <Text style={styles.previewText} numberOfLines={2}>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>{viewMode === 'list' ? 'My Birthday Cards' : 'Create Card'}</Text>
        <IconButton
          icon={viewMode === 'list' ? 'plus' : 'format-list-bulleted'}
          size={24}
          iconColor={COLORS.primary}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: COLORS.lightGrey, padding: 10 },
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  headerTitle: { fontSize: 24, fontWeight: 'bold', color: COLORS.primary },
  cardContainer: { flex: 1, backgroundColor: COLORS.white, padding: 20, marginVertical: 10, borderRadius: 10, elevation: 5 },
  decorationsContainer: { position: 'absolute', top: 0, left: 0, right: 0, height: '20%', alignItems: 'center' },
  decorationWrapper: { padding: 5 },
  image: { width: '100%', height: 200, borderRadius: 10 },
  imagePlaceholder: { width: '100%', height: 200, borderRadius: 10, backgroundColor: COLORS.lightGrey, alignItems: 'center', justifyContent: 'center' },
  placeholderText: { color: COLORS.grey, fontSize: 16 },
  textInput: { color: COLORS.grey, marginTop: 15, padding: 10, fontSize: 18 },
  toolbar: { flexDirection: 'row', marginTop: 10, justifyContent: 'space-evenly' },
  fab: { position: 'absolute', right: 20, bottom: 20, backgroundColor: COLORS.primary },
  cardPreview: { flex: 1, backgroundColor: COLORS.white, padding: 15, margin: 5, borderRadius: 10, elevation: 3, alignItems: 'center' },
  previewImage: { width: '100%', height: 100, borderRadius: 10 },
  previewText: { fontSize: 18, fontWeight: 'bold', color: COLORS.primary, marginVertical: 5 },
  dateText: { fontSize: 12, color: COLORS.grey },
  modal: { backgroundColor: COLORS.white, padding: 20, margin: 20, borderRadius: 8, elevation: 5 },
  ...
  sizeButton: { margin: 5, padding: 10, backgroundColor: COLORS.lightGrey, borderRadius: 5 },
  decorationButton: { margin: 5, padding: 10, borderRadius: 5 },
